...
class BasicAmdGpuTestCase(AmdGpuTestCaseBase):
    ...
    def get_wave_size_from_hip(self):
        """Return the wave size as reported by HIP itself.

        multi-wave.hip stores deviceProp.warpSize in a local of main(). That is
        a value from outside the debugger entirely, which makes it a useful
        cross check on the wave size the base class derives from the width of
        the exec register.

        The CPU process is left running once the GPU breakpoint is hit, so it
        has to be stopped before its frames can be read.
        """
        ...
        for i in range(cpu_thread.GetNumFrames()):
            value = cpu_thread.GetFrameAtIndex(i).FindVariable("wave_size")
            if value.IsValid() and value.GetValue() is not None:
                return value.GetValueAsUnsigned()
        self.fail("could not read wave_size from the CPU process")
    ...
    def test_wave_and_lane_ids_across_waves(self):
        ...
        # The wave size is taken from the width of the exec register. Confirm
        # against HIP's own answer, so a wrong exec width cannot quietly move
        # every expectation below with it.
        self.assertEqual(
            wave_size,
            self.get_wave_size_from_hip(),
            "exec register width should match deviceProp.warpSize",
        )
